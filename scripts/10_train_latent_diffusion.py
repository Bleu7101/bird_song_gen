...
from bird_song.generation.diffusion import ConditionalLatentDenoiser, LatentDiffusionConfig, count_parameters, diffusion_loss
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train conditional diffusion in the VQGAN latent space.")
    parser.add_argument("--vqgan-checkpoint", type=Path, required=True)
    parser.add_argument("--diffusion-config", type=Path, default=PROJECT_ROOT / "configs/latent_diffusion.json")
    ...
    parser.add_argument("--output-dir", type=Path, default=PROJECT_ROOT / "runs/latent_diffusion")
...
def evaluate(model: ConditionalLatentDenoiser, vqgan: ConditionalVQGAN, loader, device: torch.device, max_batches: int | None) -> float:
    model.eval()
    vqgan.eval()
    values: list[float] = []
    for batch_index, (real, labels, _) in enumerate(loader):
        ...
        latents = vqgan.encode(real)[0].detach()
        values.append(float(diffusion_loss(model, latents, labels)))
...
def main() -> None:
    ...
    config = LatentDiffusionConfig.from_dict(json.loads(args.diffusion_config.read_text(encoding="utf-8")))
    if (config.latent_channels, config.latent_grid) != (vq_config.latent_channels, vq_config.latent_grid):
        raise ValueError("diffusion config must match VQGAN latent shape")
    ...
    if len(classes) != config.num_classes or (spectrogram_config.n_mels, spectrogram_config.spectrogram_width) != (128, 128):
        raise ValueError("diffusion must match the existing three-class 128 x 128 representation")
    ...
    model = ConditionalLatentDenoiser(config).to(device)
    ...
    save_json(args.output_dir / "config.json", {
        **vars(args), "classes": classes, "diffusion_model": config.to_dict(), "vqgan_model": vq_config.to_dict(), "spectrogram": spectrogram_config.to_dict(), "trainable_parameters": count_parameters(model), "device_used": str(device),
    })
    scaler = torch.amp.GradScaler("cuda", enabled=device.type == "cuda")
    best_validation = float("inf")
    history_path = args.output_dir / "history.csv"
    with history_path.open("w", newline="", encoding="utf-8") as history_file:
        writer = csv.DictWriter(history_file, fieldnames=["epoch", "train_loss", "validation_loss", "lr", "seconds"])
        writer.writeheader()
        for epoch in range(1, args.epochs + 1):
            ...
            for batch_index, (real, labels, _) in enumerate(train_loader):
                ...
                with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=device.type == "cuda"):
                    loss = diffusion_loss(model, latents, labels)
            ...
            validation_loss = evaluate(model, vqgan, validation_loader, device, args.max_batches)
            row = {"epoch": epoch, "train_loss": sum(losses) / max(len(losses), 1), "validation_loss": validation_loss, "lr": optimizer.param_groups[0]["lr"], "seconds": time.perf_counter() - started}
            writer.writerow(row)
            history_file.flush()
            print(" ".join(f"{key}={value:.4f}" if isinstance(value, float) else f"{key}={value}" for key, value in row.items()))
            checkpoint = {
                "format_version": 1, "model_type": "conditional_vqgan_latent_diffusion", "model_state": model.state_dict(), "model_config": config.to_dict(), "vqgan_config": vq_config.to_dict(), "classes": list(classes), "spectrogram_config": spectrogram_config.to_dict(), "vqgan_checkpoint": str(args.vqgan_checkpoint), "epoch": epoch, "seed": args.seed, "validation_loss": validation_loss,
            }
            if validation_loss < best_validation:
                best_validation = validation_loss
                atomic_torch_save(checkpoint, args.output_dir / "best.pt")
            scheduler.step()
    print(f"Best validation diffusion loss: {best_validation:.4f}")
