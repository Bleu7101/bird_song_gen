...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate spectrograms using a VQGAN decoder and token transformer.")
    parser.add_argument("--checkpoint", type=Path, required=True)
    parser.add_argument("--vqgan-checkpoint", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, default=PROJECT_ROOT / "outputs/token_transformer")
...
def main() -> None:
    args = parse_args()
    if args.samples_per_species < 1 or args.temperature < 0:
        raise ValueError("samples-per-species must be positive and temperature cannot be negative")
    seed_everything(args.seed)
    device = choose_device(args.device)
    checkpoint = torch.load(args.checkpoint, map_location=device, weights_only=True)
    vq_checkpoint = torch.load(args.vqgan_checkpoint, map_location=device, weights_only=True)
    token_config = TokenTransformerConfig.from_dict(checkpoint["model_config"])
    ...
    for class_index, class_name in enumerate(classes):
        ...
        indices = model.generate(labels, temperature=args.temperature, generator=random_generator)
        with torch.no_grad():
            samples = vqgan.decode(indices, labels).cpu()
        ...
        for sample_index, sample in enumerate(samples):
            ...
            rows.append({"model": "conditional_vqgan_token_transformer", "species": class_name, "label_id": class_index, "sample_index": sample_index, "relative_path": path.relative_to(args.output_dir).as_posix(), "temperature": args.temperature, "seed": args.seed})
    ...
    if not args.no_figure:
        figure.suptitle(f"VQGAN + token transformer samples (temperature={args.temperature:g})")
