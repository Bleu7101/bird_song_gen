...
compare VAE and Diffusion in the maintained independent evaluation. The
...
Classifier architecture selection used validation data only. We compared the
Residual CNN, Plain CNN, Depthwise CNN, and CRNN across seeds 42, 123, and 777
using the same recording-isolated training and validation splits. The CRNN
architecture and seed-777 checkpoint were selected from validation results; for
the architecture comparison, only that checkpoint was evaluated once on the
489-clip held-out test set. VAE
evidence was separated into paired deterministic reconstruction, unpaired
generated-sample quality, and downstream classifier utility. For the auxiliary
nearest-neighbor MSE diagnostic, each generated spectrogram was compared with
its nearest same-species test spectrogram in standardized log-mel space using
a leave-one-out real reference. This diagnostic is unpaired and does not
measure perceptual realism.

For the primary low-resource experiment, each newly initialized CRNN was
trained with 50 real training spectrograms per species, selected from distinct
recording IDs. Three real-data subset choices were crossed with three CRNN
initialization seeds, yielding nine matched experimental settings. Validation
compared real-only training with adding 50, 100, or 200 generated
spectrograms per species from each generator. The CRNN architecture, masking
policy, optimizer-step budget, and validation/test splits were held constant.
No existing pretrained CRNN was augmented. Validation selected the augmentation
amount; only the shared real-only baseline and the selected 200-generated
condition for each generator were evaluated on the unchanged 489-clip test set.

Separately, each generator was timed for five synchronized in-memory repeats of
600 spectrograms (200 per species) after five warm-up batches, using FP32
precision, batch size 8, and the same RTX 4070 SUPER. Timing covered generation
through the final normalized spectrogram tensor and excluded model loading,
warm-up, conversion, CPU transfer, file output, waveform decoding, and metric
computation.
...
\begin{table}[t]
  \caption{Frozen selected-CRNN and feature-space diagnostics for generated spectrograms.}
  \label{tab:quality}
  \centering
  \small
  \resizebox{\linewidth}{!}{%
  \begin{tabular}{@{}lll@{}}
    ...
    \midrule
    Frozen selected-CRNN target-label accuracy / macro-F1 & 96.67\% / 96.66\% & 92.44\% / 92.42\% \\
...
Each condition used 50 real spectrograms per species. Validation values are
...
84.75\% for real-only, 87.48\% for VAE-v3, and 86.21\% for Diffusion. The paired changes were
...
species) per synchronized repeat after warm-up. The timing values below report
the time for the complete 600-spectrogram request; the per-spectrogram column
is the corresponding batch-average time, not an isolated single-spectrogram
latency. Pre-existing arrays are not timing evidence.
\begin{table}[t]
  ...
  \end{tabular}}
\end{table}
Diffusion took 412.0032 seconds longer per equal-count request, or
$1348.09\times$ the VAE-v3 generator time. The median [Q1, Q3] time for 600
spectrograms was 0.3046 [0.2983, 0.3076] seconds for VAE-v3 and 411.1814
[410.9396, 411.2319] seconds for Diffusion. These measurements use FP32, batch
size 8, five warm-up batches, five CUDA-synchronized repeats, and an RTX 4070
SUPER. They exclude loading, conversion, transfer, file I/O, waveform decoding,
