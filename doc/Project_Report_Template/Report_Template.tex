...
feedback, generated-to-test MSE will use each sample's nearest same-species test
spectrogram and a leave-one-out real-test baseline; diversity and nearest-train
checks are required because low MSE can also indicate copying.
...
objective. The verified portable sampler now uses
$z=\mu+0.35\,\operatorname{std}\odot\epsilon$. The checkpoint was not
retrained. The existing bank was filtered to 759 anchors in total and the
three VAE pools were regenerated; the three audited Diffusion pools were
reused without new Diffusion generation.
